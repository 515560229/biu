<template>
  <div class="app-container">
    <el-row :gutter="16">
      ...
      <el-col :span="6">
        <el-input v-model="consumerNamesQuery.key" size="mini" placeholder="输入关键字搜索" style="width: 80%;"/>
      </el-col>
      <el-col :span="6">
        <!--分页-->
        <el-pagination
      ...
      <el-table
        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            <!-- 条件面板 -->
            <template v-if="props.row.consumerConfig.parameters && props.row.consumerConfig.parameters.length > 0">
              <el-form inline>
                <el-form-item v-for="(param, index) in props.row.consumerConfig.parameters"
    ...
    <el-row style="padding: 0 auto;margin:4px -14px;">
      <el-tabs v-model="currentTabName" type="card">
        ...
        >
          ...
          <kafka-consumer-result-panel :value="tableData['val' + index]"
                                       :v-loading="tableData['loading' + index]"></kafka-consumer-result-panel>
        </el-tab-pane>
      </el-tabs>
    </el-row>
    <!--弹出窗口：新增/编辑-->
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="80%">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="120px">
        ...
        <el-form-item label="主题">
          <el-select v-model="temp.consumerConfig.topic" filterable remote reserve-keyword
        ...
        <el-form-item label="消息搜索关键字" prop="key">
          <el-input v-model="temp.consumerConfig.keyword"></el-input>
        </el-form-item>
        <template v-if="temp.consumerConfig.parameters && temp.consumerConfig.parameters.length > 0">
          <!-- 动态参数 -->
          <el-row style="font-size: 18px;">
            参数列表:
          </el-row>
          <el-form-item :label="parameter.name" v-for="(parameter,index) in temp.consumerConfig.parameters"
                        :key="'param_' + index">
...
<script>
  ...
  import KafkaConsumerResultPanel from '@/components/panel/KafkaConsumerResultPanel'

  export default {
    name: 'KafkaConsumer',
    components: {KafkaConsumerResultPanel},
    data() {
    ...
    watch: {
      //延时查询
      'consumerNamesQuery.key': debounce(function () {
    ...
    methods: {
      ...
      //数据库查询语句的相关操作
      findConsumerNames() {
        this.consumerNamesLoading = true;
        commonConfigApi.queryCommonConfig(this.consumerNamesQuery, this.page1).then(res => {
          this.consumerNamesData = res.data.page.records;
          this.consumerNamesLoading = false
          this.tableData = {};
      ...
      handleEdit(idx, entity) {
        ...
        this.findTopic(this.temp.consumerConfig.topic);
      },
      ...
      handleRowDbClick(row) {
        let _parameters = row.consumerConfig.parameters;
      ...
      deleteData(idx, dbQueryName) {
        this.$confirm('您确定要永久删除该查询语句？', '提示', confirm).then(() => {
          commonConfigApi.deleteCommonConfig(dbQueryName.id).then(res => {
            ...
            this.consumerNamesData.splice(idx, 1);
          })
        }).catch(() => {
      ...
      generateParameter() {
        ...
        const keyword = tempEntity.consumerConfig.keyword;
        let parameters = getParameters(keyword);
        ...
        //原参数由数组转成对象
        let oldParameters = tempEntity.consumerConfig.parameters;
        let oldParameterObj = oldParameters ? oldParameters.reduce((obj, currentValue) => {
          obj[currentValue.name] = currentValue;
          return obj;
        }, {}) : undefined;
        //重置 并使用新的参数信息
        tempEntity.consumerConfig.parameters = [];
        let idx = 0;
        for (let parameterName in newParameterObj) {
          if (oldParameterObj && oldParameterObj[parameterName]) {
            tempEntity.consumerConfig.parameters.splice(idx, 0, oldParameterObj[parameterName]);
          } else {
            //新的参数
            tempEntity.consumerConfig.parameters.splice(idx, 0, {
              name: parameterName,
              label: parameterName,
              defaultValue: null
            });
      ...
      executeData(idx, row) {
        ...
        kafkaApi.consumer(row).then(res => {
